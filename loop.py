...
while True:
    ...
    wakeup_time_start = Config.wakeup_time_start
    wakeup_time_end = Config.wakeup_time_end
    printer("day of week")
    printer(day_of_week)
    printer("wakeup_time")
    if (day_of_week is 5) or (day_of_week is 6):
        increment = 3
        wakeup_time_start = wakeup_time_start.replace(hour = (wakeup_time_start.hour+increment))
        wakeup_time_end = wakeup_time_end.replace(hour = (wakeup_time_end.hour+increment))
    
    printer(wakeup_time_start)
    printer( current_time)
    if debug_mode:
        alarm_periodic()
    elif current_time > wakeup_time_start and current_time < wakeup_time_end:
        alarm_periodic()
    else:
        alarm_state = "idle"
        if mixer_on:
    ...
    if current_time > Config.heater_on_time and current_time < Config.heater_off_time:
        if not heater_on:
            ser.write("2on")
            heater_on = True
            print "heating on"
